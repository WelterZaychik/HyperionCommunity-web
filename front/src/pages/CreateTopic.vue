  ...
  <div class="CreateTopicBox">
    ...
    <div class="Createbox">
      <el-steps :active="active" finish-status="success">
        <el-step title="文章/视频"></el-step>
        <el-step title="详细信息"></el-step>
        <el-step title="发布完成"></el-step>
      </el-steps>
      <el-form ref="form" :model="form" label-width="80px">

        <div class="Create01" v-show="active == 0">
          <el-button style="margin-top: 12px" @click="isvt(1)">发布视频</el-button>
          <el-button style="margin-top: 12px" @click="isvt(0)">发布文章</el-button>
        </div>

        <div class="Create02" v-show="active == 1">
      ...
      <div class="Create03" v-show="active == 2">
        <div class="enterContent">
          <div class="title"> <span>标题：</span> {{this.form.title}}</div>
          <div class="synopsis"> <span>简介：</span> {{this.form.synopsis}}</div>
          <div class="coverImagebox"> <span>缩略图：</span><p v-if="this.form.coverImage==''">未上传缩略图，或上传未成功</p>
            <div v-if="this.form.coverImage!=''" class="coverImage">  <img  :src="this.form.coverImage" alt=""></div>
          </div>
          <div class="topic-content">
            <div class="topic-content-isv">
              <div v-if="this.form.isVideo"><span>视频上传进度：</span>
                <p v-html="this.form.content==''?'上传未完成':'上传完成'"></p>
              </div>
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.form.title == "") {
        ...
        this.$message.warning("缩略图未上传（或者网络原因上传失败）");
      }else if (this.form.content == "") {
        if (this.form.isVideo) {
          this.$message.warning("视频未上传成功");
        } else {
          this.$message.warning("内容未填写哦");
        }
      } else {
        TopicApi.addTopic(this.form)
          .then((response) => {
